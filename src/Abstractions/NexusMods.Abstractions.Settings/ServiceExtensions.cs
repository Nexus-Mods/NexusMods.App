...
public static class ServiceExtensions
{
        ...
        where T : class, ISettings, new()
    {
        ...
        object Hack(IServiceProvider provider, object obj) => overrideMethod((T)obj);
    }
    ...
    /// for tests.
    ///
    /// This variant also includes the <see cref="IServiceCollection"/> in the lambda
    /// so you can lookup other services if needed.
    /// </remarks>
    public static IServiceCollection OverrideSettingsForTests<T>(
        this IServiceCollection serviceCollection,
        Func<IServiceProvider, T, T> overrideMethod)
        where T : class, ISettings, new()
    {
        return serviceCollection.AddSingleton(new SettingsOverrideInformation(typeof(T), Hack));

        object Hack(IServiceProvider provider, object obj) => overrideMethod(provider, (T)obj);
    }
