...
public class StressTest
{
        ...
        [Injected] CancellationToken token)
    {
        ...
        var domain = domainToIdCache[game.NexusModsGameId.Value].Value;
        ...
        try
        {
            foreach (var mod in mods.Data)
            {
                ...
                try
                {
                    files = await nexusGqlClient.QueryModFiles.ExecuteAsync(mod.ModId.ToString(), game.NexusModsGameId.Value.ToString(), token);
                ...
                foreach (var file in files.Data!.ModFiles.Where(f => Size.FromLong(long.Parse(f.SizeInBytes ?? "0")) < MaxFileSize))
                {
                    ...
                    try
                    {
                        ...
                        var downloadJob = await nexusModsLibrary.CreateDownloadJob(tmpPath, game.NexusModsGameId.Value, mod.ModId, uid.FileId, cancellationToken: CancellationToken.None);
