...
public interface ILoadoutSortableItemProvider : IDisposable
{
    ...
    /// </summary>
    public IObservable<IChangeSet<IReactiveSortItem, ISortItemKey>> SortableItemsChangeSet { get; }
    ...
    public IReadOnlyList<IReactiveSortItem> GetCurrentSorting();

    /// <summary>
    /// Returns the sortable item with the given id
    /// </summary>
    public Optional<IReactiveSortItem> GetSortableItem(ISortItemKey itemId);

    /// <summary>
    /// Sets the relative position of a sortable item in the sort order
    /// </summary>
    /// <param name="reactiveSortItem">item to move</param>
    /// <param name="delta">positive or negative index delta</param>
    /// <param name="token"></param>
    Task SetRelativePosition(IReactiveSortItem reactiveSortItem, int delta, CancellationToken token);
    ...
    /// </summary>
    Task MoveItemsTo(IReactiveSortItem[] sourceItems, IReactiveSortItem targetItem, TargetRelativePosition relativePosition, CancellationToken token);
    ...
    Task<IReadOnlyList<IReactiveSortItem>> RefreshSortOrder(CancellationToken token, IDb? loadoutDb = null);
}
...
{
    /// <inheritdoc/>
    IObservable<IChangeSet<IReactiveSortItem, ISortItemKey>> ILoadoutSortableItemProvider.SortableItemsChangeSet => SortableItemsChangeSet.ChangeKey((key, _) => (ISortItemKey)key).Transform(item => (IReactiveSortItem)item);
    ...
    /// <inheritdoc/>
    IReadOnlyList<IReactiveSortItem> ILoadoutSortableItemProvider.GetCurrentSorting() => GetCurrentSorting().Cast<IReactiveSortItem>().ToList();
    
    /// <inheritdoc cref="ILoadoutSortableItemProvider.GetCurrentSorting"/>
    new IReadOnlyList<TItem> GetCurrentSorting();

    /// <inheritdoc/>
    Optional<IReactiveSortItem> ILoadoutSortableItemProvider.GetSortableItem(ISortItemKey itemId) => GetSortableItem((TKey)itemId).Convert(x => (IReactiveSortItem)x);
    
    /// <inheritdoc cref="ILoadoutSortableItemProvider.GetSortableItem"/>
    Optional<TItem> GetSortableItem(TKey itemId);

    /// <inheritdoc/>
    Task ILoadoutSortableItemProvider.SetRelativePosition(IReactiveSortItem reactiveSortItem, int delta, CancellationToken cancellation) => SetRelativePosition((TItem)reactiveSortItem, delta, cancellation); 
    ...
    /// <inheritdoc/>
    Task ILoadoutSortableItemProvider.MoveItemsTo(IReactiveSortItem[] sourceItems, IReactiveSortItem targetItem, TargetRelativePosition relativePosition, CancellationToken token) => MoveItemsTo(sourceItems.Cast<TItem>().ToArray(), (TItem)targetItem, relativePosition, token);
    ...
    /// <inheritdoc/>
    async Task<IReadOnlyList<IReactiveSortItem>> ILoadoutSortableItemProvider.RefreshSortOrder(CancellationToken token, IDb? loadoutDb) => (await RefreshSortOrder(token, loadoutDb)).Cast<IReactiveSortItem>().ToList();
