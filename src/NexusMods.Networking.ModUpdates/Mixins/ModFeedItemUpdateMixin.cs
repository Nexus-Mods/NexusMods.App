...
public readonly struct ModFeedItemUpdateMixin : IModFeedItem
{
    ...
    /// <summary/>
    private ModFeedItemUpdateMixin(ModUpdate update, NexusModsGameId nexusModsGameId)
    {
    ...
    /// </summary>
    public static IEnumerable<ModFeedItemUpdateMixin> FromUpdateResults(IEnumerable<ModUpdate> updates, NexusModsGameId nexusModsGameId) => updates.Select(update => new ModFeedItemUpdateMixin(update, nexusModsGameId));

    /// <inheritdoc />
    public DateTimeOffset GetLastUpdatedDate() => _lastUpdatedDate;

    /// <inheritdoc />
    public ModUid GetModPageId() => new(_modId, _nexusModsGameId);
}
