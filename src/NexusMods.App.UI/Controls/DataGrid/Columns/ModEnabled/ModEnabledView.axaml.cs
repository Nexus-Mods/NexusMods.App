...
public partial class ModEnabledView : ReactiveUserControl<IModEnabledViewModel>
{
    public ModEnabledView()
    {
        ...
        this.WhenActivated(d =>
        {
            ...
            var isCheckedObservable = this
                .WhenAnyValue(view => view.EnabledToggleSwitch.IsChecked)
                .Select(isChecked => isChecked ?? false);

            this.BindCommand(ViewModel, vm => vm.ToggleEnabledCommand, view => view.EnabledToggleSwitch, isCheckedObservable)
                .DisposeWith(d);
        });
    }
}
