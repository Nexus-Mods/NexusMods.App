...
internal class LocalFileDataProvider : ILibraryDataProvider
{
    ...
    public IObservable<IChangeSet<LibraryItemModel>> ObserveNestedLibraryItems()
    {
        return _connection
            ...
            {
                var libraryFile = LibraryFile.Load(_connection.Db, datom.E);
                var libraryItem = ToLibraryItemModel(libraryFile);

                var hasChildrenObservable = Observable.Return(true);
                var childrenObservable = Observable.Return(new ChangeSet<LibraryItemModel>([new Change<LibraryItemModel>(ListChangeReason.Add, libraryItem)]));
                ...
                return new LibraryItemModel
                {
                    ...
                    LinkedLoadoutItemsObservable = libraryItem.LinkedLoadoutItemsObservable,
                };
            });
    }
}
