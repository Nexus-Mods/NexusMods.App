...
{
    public PreviewTreeEntryViewModel? Parent { get; init; }
    ...
    public PreviewTreeEntryViewModel(GamePath fullPath, PreviewEntryNodeFlags flags, PreviewTreeEntryViewModel? parent = null)
    {
        Parent = parent;
        FullPath = fullPath;
        Flags = flags;
    ...
    private void UnlinkRecursive(bool isCalledFromDoubleLinkedItem)
    {
        ...
        {
            var node = child as PreviewTreeEntryViewModel;
    ...
    public static PreviewTreeEntryViewModel Create(GamePath fullPath, bool isDirectory)
    {
        var root = new PreviewTreeEntryViewModel(new GamePath(fullPath.LocationId, ""),
        ...
        where TChecker : struct, ICheckIfItemAlreadyExists // for devirtualization, do not de-struct.
    {
        ...
        for (var x = 0; x < pathComponents.Length; x++)
        {
            ...
            if (childNode == null)
            {
                ...
                childNode = new PreviewTreeEntryViewModel(newGamePath, isNewFlag | isDirectoryFlag, currentNode);
                currentNode.Children.Add(childNode);
            }

            // Set the current node to the child node and continue.
            currentNode = (PreviewTreeEntryViewModel)childNode;
        }
    }
    ...
    public IPreviewTreeEntryViewModel? GetChild(string relativePath)
    {
        if (relativePath == "")
            return this;
