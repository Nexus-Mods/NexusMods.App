...
/// <summary>
///     Represents a <see cref="IModContentTreeEntryViewModel" /> that is backed by a
...
internal class ModContentTreeEntryViewModel<TNodeValue> : AViewModel<IModContentTreeEntryViewModel>, IModContentTreeEntryViewModel
{
    /// <summary>
    ...
    /// </remarks>
    public required ModContentTreeEntryViewModel<TNodeValue>? Parent { get; init; }
    ...
    /// <inheritdoc />
    public required IModContentTreeEntryViewModel[] Children { get; init; }
    ...
    protected ModContentTreeEntryViewModel()
    {
    ...
    public void Link(DeploymentData data, IModContentBindingTarget target, bool targetAlreadyExisted)
    {
        ...
        {
            var node = child as ModContentTreeEntryViewModel<TNodeValue>;
    ...
    private static void LinkRecursive(ModContentTreeEntryViewModel<TNodeValue> @this, DeploymentData data,
    ...
    /// </remarks>
    public IEnumerable<ModContentTreeEntryViewModel<TNodeValue>> ChildrenFlattened()
    {
        var stack = new Stack<ModContentTreeEntryViewModel<TNodeValue>>();

        // Push initial children onto the stack.
        foreach (var child in Children)
            stack.Push((child as ModContentTreeEntryViewModel<TNodeValue>)!);
    ...
    /// </summary>
    private static void SetStatusRecursive(ModContentTreeEntryViewModel<TNodeValue> item, ModContentNodeStatus status)
    {
        foreach (var childInterface in item.Children)
        {
            // Covariant cast to remove virtualization and make Status writeable.
            var child = childInterface as ModContentTreeEntryViewModel<TNodeValue>;
    ...
    /// <summary>
    ///     Creates a new <see cref="ModContentModContentTreeEntryViewModel{TNodeValue}" /> from a given
    ...
    public static ModContentTreeEntryViewModel<TNodeValue> FromFileTree(FileTreeNode<RelativePath, TNodeValue> node,
        IAdvancedInstallerCoordinator coordinator)
    {
        var root = new ModContentTreeEntryViewModel<TNodeValue>
        {
            Node = node,
            Parent = null!,
            Children = GC.AllocateUninitializedArray<IModContentTreeEntryViewModel>(node.Children.Count),
            IsTopLevel = false,
    ...
    /// <summary>
    ///     Recursively creates new <see cref="ModContentModContentTreeEntryViewModel{TNodeValue}" /> entries from a given matching
    ...
    private static IModContentTreeEntryViewModel FromFileTreeRecursive(FileTreeNode<RelativePath, TNodeValue> node,
        ModContentTreeEntryViewModel<TNodeValue> parent)
    {
        var item = new ModContentTreeEntryViewModel<TNodeValue>
        {
            Node = node,
            Parent = parent,
            Children = GC.AllocateUninitializedArray<IModContentTreeEntryViewModel>(node.Children.Count),
...
/// <summary>
///     Represents the current status of the <see cref="ModContentModContentTreeEntryViewModel{TNodeValue}" />.
/// </summary>
