...
public class LoadoutQueriesTests(ITestOutputHelper helper) : ACyberpunkIsolatedGameTest<LoadoutQueriesTests>(helper)
{
    ...
    [Fact]
    public async Task IsCollectionEnabledQuery_Test()
    {
        ...
        {
            var collectionIsEnabled = Loadout.IsCollectionEnabled(Connection, collection.Id);
            collectionIsEnabled.HasValue.Should().BeTrue();
            collectionIsEnabled.Value.Should().BeTrue();
        }
        
        // Disable the collection
        using (var tx = Connection.BeginTransaction())
        {
            tx.Add(collection.Id,
...

        {
            var collectionIsEnabled = Loadout.IsCollectionEnabled(Connection, collection.Id);
            collectionIsEnabled.HasValue.Should().BeTrue();
            collectionIsEnabled.Value.Should().BeFalse();
        }
    }
    
    [Fact]
    public async Task IsLoadoutItemGroupEnabledQuery_Test()
    {
        var loadout = await CreateLoadout();
        ...
        {
            var groupIsEnabled = Loadout.IsLoadoutItemGroupEnabled(Connection, group.Id);
            groupIsEnabled.HasValue.Should().BeTrue();
            groupIsEnabled.Value.Should().BeTrue();
        }
        ...
        // Check if the group is disabled
        {
            var groupIsEnabled = Loadout.IsLoadoutItemGroupEnabled(Connection, group.Id);
            groupIsEnabled.HasValue.Should().BeTrue();
            groupIsEnabled.Value.Should().BeFalse();
        }
        ...
        // Check if the group is still disabled (due to collection being disabled)
        {
            var groupIsEnabled = Loadout.IsLoadoutItemGroupEnabled(Connection, group.Id);
            groupIsEnabled.HasValue.Should().BeTrue();
            groupIsEnabled.Value.Should().BeFalse();
    ...
    [Fact]
    public async Task IsLoadoutItemEnabledQuery_Test()
    {
        var loadout = await CreateLoadout();
        var collection1 = await CreateCollection(loadout.LoadoutId, "Collection 1");
        ...
        {
            var itemIsEnabled = Loadout.IsLoadoutItemEnabled(Connection, item.Id);
            itemIsEnabled.HasValue.Should().BeTrue();
            itemIsEnabled.Value.Should().BeTrue();
        }
        ...
        // Check if the item is disabled
        {
            var itemIsEnabled = Loadout.IsLoadoutItemEnabled(Connection, item.Id);
            itemIsEnabled.HasValue.Should().BeTrue();
            itemIsEnabled.Value.Should().BeFalse(); 
        }
        ...
        // Check if the item is still disabled (due to group being disabled)
        {
            var itemIsEnabled = Loadout.IsLoadoutItemEnabled(Connection, item.Id);
            itemIsEnabled.HasValue.Should().BeTrue();
            itemIsEnabled.Value.Should().BeFalse();
        ...
        // Check if the item is still disabled (due to collection being disabled)
        {
            var itemIsEnabled = Loadout.IsLoadoutItemEnabled(Connection, item.Id);
            itemIsEnabled.HasValue.Should().BeTrue();
            itemIsEnabled.Value.Should().BeFalse(); 
