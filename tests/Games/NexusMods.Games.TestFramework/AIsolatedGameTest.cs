...
public abstract class AIsolatedGameTest<TTest, TGame> : IAsyncLifetime where TGame : AGame
{
    ...
    /// </summary>
    protected LoadoutItemGroupId AddEmptyGroup(ITransaction tx, LoadoutId loadoutId, string name, LoadoutItemGroupId? parentGroup = null)
    {
    ...
    /// </param>
    public async Task<(AbsolutePath archivePath, List<Hash> hashes)> AddModAsync(ITransaction tx, IEnumerable<RelativePath> paths, LoadoutId loadoutId, string modName, LibraryArchive.ReadOnly? libraryArchive = null, LoadoutItemGroupId? parentGroup = null)
    {
